function funcOut = trap_effective_volume(analyVar, indivDataset, avgDataset)
...
fug = double(vpasolve(polylog_BEC_fermi(-fugacity)+1/(6*(T/Tf)^3)==0,fugacity,[0,1]));
fugbose =double(vpasolve(polylog_BEC_fermi(fugacity)==num*((hbar*((omega_r^2)*(omega_dimple))^(1/3))/(kb*T))^3,fugacity,[0,1]));
fugunpol =double(vpasolve(polylog_BEC_fermi(-fugacity)+1/(6*(T/Tfunpol)^3)==0,fugacity,[0,1]));;

%%%%%%%%%%% Functions %%%%%%%%%%%%
xmini=-40*10^(-6);  % This sets the limits of the integral ; make sure it covers the whole cloud
xmaxi=40*10^(-6);

densityfermi = @(x,y,z) -(kb*m*T)^(3/2)/((2*pi)^3/2*hbar^3).*polylog_BEC_fermi(-fug.*exp((-m*omega_r^2.*(x.^2 + y.^2 + lambda^2.*z.^2))./(2*kb*T)));
densityfermiunpol = @(x,y,z) -(kb*m*T)^(3/2)/((2*pi)^3/2*hbar^3).*polylog_BEC_fermi(-fugunpol.*exp((-m*omega_r^2.*(x.^2 + y.^2 + lambda^2.*z.^2))./(2*kb*T)));
densitybose =  @(x,y,z) (kb*m*T)^(3/2)/((2*pi)^3/2*hbar^3).*polylog_BEC_fermi(fugbose.*exp((-m*omega_r^2.*(x.^2 + y.^2 + lambda^2.*z.^2))./(2*kb*T))); 
...
%%%%%%%%%% Effective Volume integrals %%%%%%%%%%%
    for i=1:3
        vbose{i} =integral3(@(x,y,z) densitybose(x,y,z).^i,xmini,xmaxi,xmini,xmaxi,xmini,xmaxi)/(densitybose(0,0,0))^i;
        vfermi{i} =integral3(@(x,y,z) densityfermi(x,y,z).^i,xmini,xmaxi,xmini,xmaxi,xmini,xmaxi)/densityfermi(0,0,0)^i;
        vfermiunpol{i} =integral3(@(x,y,z) densityfermiunpol(x,y,z).^i,xmini,xmaxi,xmini,xmaxi,xmini,xmaxi)/densityfermiunpol(0,0,0)^i;
    end
