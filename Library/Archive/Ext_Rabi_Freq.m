...
% Rabi Frequency 
rabiFreqFunc = @(amplitude,expTime) (2*pi).^(5/4).*sqrt(amplitude/expTime);
...
for basenameNum = 1:analyVar.numBasenamesAtom
    ...
    rabiFreq(basenameNum,1)   = rabiFreqFunc(double(specFitModel.Coefficients('Amplitude','Estimate')),expTime);
    rabiFreq(basenameNum,2)   = rabiFreqFunc(double(specFitModel.Coefficients('Amplitude','SE')),expTime);
