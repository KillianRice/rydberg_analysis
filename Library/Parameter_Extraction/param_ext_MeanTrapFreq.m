function avgDataset = param_ext_MeanTrapFreq(analyVar,indivDataset,avgDataset)
...
%                  avgTrapFreq  - Average trap frequency
%
% NOTE:
...
for uniqScanIter = 1:length(analyVar.uniqScanList);
    ...
    for simScanIter = 1:length(analyVar.posOccurUniqVar{uniqScanIter})
        ...
        trapFreq  = indivDataset{basenameNum}.atomTempX;
        ...
        avgTrapFreq(idxSharedWithSim,simScanIter) = trapFreq(1,idxSharedInBatch);
    end
    
    %% Average all like points together
    % Function below takes the mean of the matrix of mixed NaN's and values
    avgDataset{uniqScanIter}.avgTrapFreq = nanmean(avgTrapFreq,2);
end
end