...
function an = myAnnotate(coeffs)
    ...
    dim = [.7 .5 .3 .3];
    fmt = '%0.2e';
    str_mean = strcat('N_0: ',num2str(coeffs(1),'%.1f'));
    str_std = strcat('\Gamma_{AI}: ',num2str(coeffs(2)*1e-6,fmt),' \mus^{-1}');
    str_offset = strcat('\Gamma_{L}: ',num2str(coeffs(3)*1e-6,fmt),' \mus^{-1}');
    str_int = strcat('\tau_{R}: ',num2str(1/coeffs(4)*1e6,'%.0f'),' \mus');
